...
pub trait Interface {
    ...
    /// Send a pre-formatted pixel buffer (u8 bytes)
    ///
    /// This is the fastest method for sending large amounts of pixel data
    /// as it avoids per-pixel conversion overhead.
    ///
    /// `WriteMemoryStart` must be sent before calling this function
    ...
    /// Send a pre-formatted pixel buffer (u16 words)
    ///
    /// This is the fastest method for RGB565 format on platforms that support
    /// 16-bit transfers, avoiding byte-order conversion overhead.
    ///
    /// `WriteMemoryStart` must be sent before calling this function
