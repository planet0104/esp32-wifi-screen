    ...
    RST: OutputPin,
{
    ...
    /// Fast pixel drawing using pre-formatted u8 buffer (ZERO-COPY)
    ///
    /// This is the fastest method for sending large amounts of pixel data
    /// as it avoids per-pixel conversion overhead and directly writes
    /// the buffer to SPI.
    ///
    /// The buffer must be in the correct format for the display's color mode:
    /// - RGB565: 2 bytes per pixel (big-endian: [r4r3r2r1r0g5g4g3, g2g1g0b4b3b2b1b0])
    /// - RGB666: 3 bytes per pixel
    ///
    /// <div class="warning">
    ///
    /// The end values of the X and Y coordinate ranges are inclusive, and no
    /// bounds checking is performed on these values.
    ///
    /// </div>
    ...
    /// that support 16-bit SPI transfers, as it avoids both per-pixel conversion
    /// and byte-order conversion overhead.
    ///
    /// The buffer must contain RGB565 pixels in native u16 format.
    ///
    /// <div class="warning">
    ///
    /// The end values of the X and Y coordinate ranges are inclusive, and no
    ...
    /// Sets pixels using RAW framebuffer coordinates (bypassing display_offset)
    /// 
    /// This is useful for clearing the entire framebuffer or accessing areas
    /// outside the configured display_size.
    /// 
    /// CAUTION: This bypasses all offset and bounds checking!
    ...
    /// Fast pixel drawing using RAW framebuffer coordinates and u16 buffer (bypassing display_offset)
    pub fn set_pixels_buffer_u16_raw(
        &mut self,
        sx: u16,
        sy: u16,
        ex: u16,
