    ...
    WR: OutputPin,
{
    ...
    fn send_pixels_buffer(
        ...
        // For parallel interface, convert u8 to BUS::Word and send
        for &byte in pixels {
            self.send_word(BUS::Word::from(byte))?;
        }
        Ok(())
    }
    ...
    fn send_pixels_buffer_u16(
        ...
        // For parallel interface with u16 pixels, send as bytes
        use byte_slice_cast::AsByteSlice;
        let bytes = pixels.as_byte_slice();
        for &byte in bytes {
            self.send_word(BUS::Word::from(byte))?;
        }
        Ok(())
